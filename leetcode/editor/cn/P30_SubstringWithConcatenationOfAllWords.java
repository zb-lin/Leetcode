...
// 
//
// Related Topics 哈希表 字符串 滑动窗口 👍 1110 👎 0
...
public class P30_SubstringWithConcatenationOfAllWords {
    ...
    class Solution {
        ...
        /*Set<String> fullyArranged = new HashSet<>();
        boolean[] marked;

        public List<Integer> findSubstring(String s, String[] words) {
            List<Integer> res = new ArrayList<>();
            marked = new boolean[words.length];
            getFullyArranged(words, new StringBuilder());
            int length = words[0].length() * words.length;
            for (int i = 0; i < s.length() - length + 1; i++) {
                if (fullyArranged.contains(s.substring(i, i + length))) {
                    res.add(i);
                }
            }
            return res;
        ...
        public void getFullyArranged(String[] words, StringBuilder sb) {
            if (sb.length() == words[0].length() * words.length) {
                fullyArranged.add(sb.toString());
                return;
            }
            for (int i = 0; i < words.length; i++) {
                ...
                sb.append(words[i]);
                marked[i] = true;
                getFullyArranged(words, sb);
                marked[i] = false;
                sb.delete(sb.length() - words[0].length(), sb.length());
            }
        }*/
    }
